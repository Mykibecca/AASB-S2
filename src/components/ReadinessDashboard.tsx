  ...
  return (
    <Card className="shadow-card">
      <CardHeader>
        ...
        <CardDescription>Live scoring with skip-logic applied</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <div className="text-3xl font-bold"><Badge variant="outline">{scoring?.readiness ?? '—'}</Badge></div>
            <div className="text-sm text-muted-foreground">Overall readiness</div>
          </div>
          <div>
            <div className="text-3xl font-bold">{(scoring?.sectionScores || []).reduce((a,b)=>a + b.sectionScore, 0).toFixed(2)}</div>
            <div className="text-sm text-muted-foreground">Sum of sections</div>
          </div>
          <div>
            <div className="text-3xl font-bold"><Badge variant="outline">{scoring?.readiness ?? '—'}</Badge></div>
        ...
        {/* Urgency Breakdown */}
        {scoring?.urgencyBreakdown && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-3 bg-red-50 rounded-lg border border-red-200">
              <div className="text-2xl font-bold text-red-600">{scoring.urgencyBreakdown.High}</div>
              <div className="text-sm text-red-700">High Priority</div>
            </div>
            <div className="text-center p-3 bg-amber-50 rounded-lg border border-amber-200">
              <div className="text-2xl font-bold text-amber-600">{scoring.urgencyBreakdown.Medium}</div>
              <div className="text-sm text-amber-700">Medium Priority</div>
            </div>
            <div className="text-center p-3 bg-green-50 rounded-lg border border-green-200">
              <div className="text-2xl font-bold text-green-600">{scoring.urgencyBreakdown.Low}</div>
